...
class RestoreEndpoint extends Endpoint {
  Future<Document> restoreDocument(Session session, UuidValue documentId) async {
    if (!session.canWrite) {
  ...
  Future<Project> restoreProject(Session session, UuidValue projectId) async {
  ...
  Future<User> restoreUser(Session session, UuidValue userId) async {
    if (session.authenticated == null) {
      throw ApiException(message: 'Authentication required', code: 401);
    }
    await RoleGuard.requireRole(
